                    ...
                    short* seqB_align_begin, short* seqB_align_end, short* top_scores)
{
    ...
    __syncthreads(); // Mutation e2: -c U616
    for(int diag = 0; diag < lengthSeqA + lengthSeqB - 1; diag++)
    {  // iterate for the number of anti-diagonals
        ...
        memset(curr_H, 0, (minSize + 1) * sizeof(short)); // Mutation e3: -c U884: comment out
        __syncthreads(); // Mutation e4 -c U885: comment out
        tmp_ptr     = prev_E;
        prev_E      = curr_E;
        curr_E      = prev_prev_E;
        prev_prev_E = tmp_ptr;

        memset(curr_E, 0, (minSize + 1) * sizeof(short)); // Mutation e5: -c U887: comment out
        __syncthreads(); // Mutation i1: -c U888
        tmp_ptr     = prev_F;
        prev_F      = curr_F;
        curr_F      = prev_prev_F;
        prev_prev_F = tmp_ptr;

        memset(curr_F, 0, (minSize + 1) * sizeof(short)); // Mutation e6: -c U890: comment out

        __syncthreads(); // Mutation e7: -c U891: comment out
        ...
        if(is_valid[myTId] && myTId < minSize)
        {
            ...
            curr_F[j] = (fVal > hfVal) ? fVal : hfVal; // Mutation e8, e13: -c U912, -p U911.OP0,U381: comment out
            curr_E[j] = (eVal > heVal) ? eVal : heVal; // Mutation i2: -p U913.OP1,U905, e9: -c U915 -p U916.OP1,U455 -p U927.OP0,U455
            // *I_i = eVal; // curr_E[j] = eVal; // Mutation i2 e9
            ...
            traceback[1] = curr_F[j]; // Mutation e10: -c U930 -p U931.OP0,U928 -p U943.OP0,U928
            // traceback[1] = -1;  // Mutation e10
            traceback[2] = curr_E[j]; // Mutation e9: -c U915 -p U916.OP1,U455 -p U927.OP0,U455
            // traceback[2] = *I_i; // Mutation e9
            traceback[3] = 0;
            ...
            thread_max_j = (thread_max >= curr_H[j]) ? thread_max_j : myTId + 1; // Mutation i3: -p U970.OP0,U618:
